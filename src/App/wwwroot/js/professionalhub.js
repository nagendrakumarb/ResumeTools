window.professionalHub = {
  ...
  images: {
    analyzeTemplate: function (base64, contentType) {
      return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
          const width = 240, height = Math.max(1, Math.round(width * img.height / img.width));
          ...
          const bands = [0, 0, 0], colorBands = [0, 0, 0], topBands = [0, 0, 0];
          for (let y = 0; y < height; y += 2) for (let x = 0; x < width; x += 2) {
            const i = (y * width + x) * 4, rr = data[i], gg = data[i + 1], bb = data[i + 2];
            const max = Math.max(rr, gg, bb), min = Math.min(rr, gg, bb), saturation = max - min;
            if (max < 238 || saturation > 22) {
              ink++;
              const band = Math.min(2, Math.floor(x / (width / 3)));
              bands[band]++;
              if (y < height * .18) topBands[band]++;
            }
          ...
          for (let y = 0; y < height; y++) {
            ...
            if (longest > width * .48) ruleRows++;
          }
          const hex = n => Math.round(n).toString(16).padStart(2, "0").toUpperCase();
          const hasColor = count > Math.max(20, ink * .025);
          const accentHex = hasColor ? `${hex(r / count)}${hex(g / count)}${hex(b / count)}` : "111111";
          const edgeMax = Math.max(bands[0], bands[2]), edgeMin = Math.min(bands[0], bands[2]);
          ...
          const sidebar = !twoColumn ? "none" :
            coloredEdge > count * .2 ? (colorBands[0] > colorBands[2] ? "left" : "right") :
            (bands[0] < bands[2] ? "left" : "right");
          const topTotal = topBands[0] + topBands[1] + topBands[2] || 1;
          const centeredHeader = topBands[1] / topTotal > .38;
          resolve({
            accentHex,
  ...
  jobLedger: {
    ...
    getState: async function () {
      if (!window.showDirectoryPicker)
        return { supported: false, configured: false, permissionGranted: false, message: "Folder storage requires a Chromium browser such as Edge or Chrome." };
      const handle = await this.getValue("settings", "jobLedgerFolder");
      if (!handle)
        return { supported: true, configured: false, permissionGranted: false, message: "Choose a folder to store encrypted applied-job records." };
      const granted = await this.permission(handle, false);
      return {
        supported: true, configured: true, permissionGranted: granted,
        message: granted ? `Mapped folder: ${handle.name}` : `Folder ${handle.name} is mapped; click Map local folder to restore permission.`
      };
    },
    ...
    decrypt: async function (chunk, key) {
      const plain = await crypto.subtle.decrypt(
        { name: "AES-GCM", iv: this.fromBase64(chunk.iv) }, key, this.fromBase64(chunk.cipher));
    ...
    readDateFolder: async function (directory, key) {
      try {
        ...
        for (const chunk of envelope.chunks || []) {
          try { records.push(await this.decrypt(chunk, key)); }
          catch (error) { console.warn("Skipped an unreadable applied-job chunk.", error); }
        }
    ...
    save: async function (records) {
      ...
      for (const [date, incoming] of groups) {
        ...
        for (const record of incoming) {
          if (byId.has(record.fingerprint)) updated++; else saved++;
        ...
        const chunks = [];
        for (const record of byId.values()) chunks.push(await this.encrypt(record, ledger.key));
  ...
  history: {
    open: function () { return new Promise((resolve, reject) => { const request = indexedDB.open("professional-hub", 2); request.onupgradeneeded = () => { if (!request.result.objectStoreNames.contains("analyses")) request.result.createObjectStore("analyses", { keyPath: "id", autoIncrement: true }); }; request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); },
    save: async function (record) { const db = await this.open(); const transaction = db.transaction("analyses", "readwrite"); transaction.objectStore("analyses").add(record); return new Promise((resolve, reject) => { transaction.oncomplete = resolve; transaction.onerror = () => reject(transaction.error); }); },
    getAll: async function () { const db = await this.open(); const request = db.transaction("analyses", "readonly").objectStore("analyses").getAll(); return new Promise((resolve, reject) => { request.onsuccess = () => resolve(request.result); request.onerror = () => reject(request.error); }); }
  }
};
